...
export function MarketplaceCart({ open, items, order, busy, onClose, onRemove, onCheckout }: { open: boolean; items: MarketplaceCartItem[]; order: PublicOrder | null; busy: boolean; onClose: () => void; onRemove: (productId: string) => void; onCheckout: () => void }) {
  if (!open) return null;
  const qrImg = order ? (
    (order.paymentQrUrl && (order.paymentQrUrl.startsWith('data:') || order.paymentQrUrl.startsWith('/uploads/') || order.paymentQrUrl.includes('qrserver') || order.paymentQrUrl.includes('googleapis'))) 
      ? order.paymentQrUrl 
      : order.qrisPayload 
        ? `https://api.qrserver.com/v1/create-qr-code/?size=260x260&data=${encodeURIComponent(order.qrisPayload)}`
        : order.sakuRupiahCheckoutUrl 
          ? `https://api.qrserver.com/v1/create-qr-code/?size=260x260&data=${encodeURIComponent(order.sakuRupiahCheckoutUrl)}`
          : order.paymentQrUrl
  ) : undefined;

  return <div className="cart-backdrop" onMouseDown={onClose}><aside className="cart-drawer" onMouseDown={(event) => event.stopPropagation()}><header><div><small>KERANJANG BELANJA</small><h2>{items.length} produk dipilih</h2></div><button onClick={onClose}><X /></button></header>{order ? <div className="cart-invoice"><BadgeCheck size={42} /><h3>Invoice {order.invoiceNumber}</h3><p>Bayar tepat sesuai nominal berikut:</p><strong>{order.formattedTotalAmount}</strong>{qrImg && <div className="qris-display-box" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', margin: '14px 0', background: '#fff', padding: '12px', borderRadius: '12px', border: '1px solid var(--line)' }}><img src={qrImg} alt="QRIS Pembayaran" style={{ width: '220px', height: '220px', objectFit: 'contain' }} /><small className="muted" style={{ marginTop: '6px', fontSize: '11px', textAlign: 'center' }}>Scan QRIS di atas menggunakan aplikasi E-Wallet / Mobile Banking kamu</small></div>}{order.sakuRupiahCheckoutUrl && <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '8px' }}><a className="buy-primary" href={order.sakuRupiahCheckoutUrl} target="_blank" rel="noreferrer" style={{ padding: '12px 18px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', textDecoration: 'none', fontWeight: '700', borderRadius: '10px', fontSize: '13px' }}>Bayar via Web Gateway SakuRupiah <ExternalLink size={16} /></a></div>}<a href="/member" style={{ marginTop: '12px', display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: '600' }}>Buka Area Member <ArrowRight size={16} /></a></div> : <>{items.length ? <div className="cart-items">{items.map((item) => <article key={item.productId}><div>{item.coverUrl ? <img src={item.coverUrl} alt="" /> : <ShoppingCart />}</div><span><b>{item.productName}</b><small>{item.planName}</small><strong>{money(item.price)}</strong></span><button onClick={() => onRemove(item.productId)}><Trash2 /></button></article>)}</div> : <div className="cart-empty"><ShoppingCart /><h3>Keranjang masih kosong</h3><p>Tambahkan produk yang ingin dibeli.</p></div>}<footer><span><small>Subtotal</small><b>{money(cartSubtotal(items))}</b></span><button disabled={!items.length || busy} onClick={onCheckout}>{busy ? 'Membuat invoice...' : 'Checkout Sekarang'} <ArrowRight /></button><small>HWID lisensi dimasukkan di halaman member setelah pembayaran disetujui.</small><small>Satu invoice dan satu QRIS untuk semua produk.</small></footer></>}</aside></div>;
}
