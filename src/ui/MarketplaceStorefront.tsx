...
export function MarketplaceCart({ open, items, order, busy, onClose, onRemove, onCheckout }: { open: boolean; items: MarketplaceCartItem[]; order: PublicOrder | null; busy: boolean; onClose: () => void; onRemove: (productId: string) => void; onCheckout: (hwid?: string) => void }) {
  const [hwid, setHwid] = useState('');
  if (!open) return null;
  const needsHwid = items.some((item) => item.fulfillmentType === 'license');
  return <div className="cart-backdrop" onMouseDown={onClose}><aside className="cart-drawer" onMouseDown={(event) => event.stopPropagation()}><header><div><small>KERANJANG BELANJA</small><h2>{items.length} produk dipilih</h2></div><button onClick={onClose}><X /></button></header>{order ? <div className="cart-invoice"><BadgeCheck size={42} /><h3>Invoice {order.invoiceNumber}</h3><p>Bayar tepat sesuai nominal berikut:</p><strong>{order.formattedTotalAmount}</strong>{order.paymentQrUrl && <img src={order.paymentQrUrl} alt="QRIS pembayaran" />}<small>Setelah membayar, kirim bukti dari halaman member. Admin akan memverifikasi manual.</small><a href="/member">Buka Area Member <ArrowRight /></a></div> : <>{items.length ? <div className="cart-items">{items.map((item) => <article key={item.productId}><div>{item.coverUrl ? <img src={item.coverUrl} alt="" /> : <ShoppingCart />}</div><span><b>{item.productName}</b><small>{item.planName}</small><strong>{money(item.price)}</strong></span><button onClick={() => onRemove(item.productId)}><Trash2 /></button></article>)}</div> : <div className="cart-empty"><ShoppingCart /><h3>Keranjang masih kosong</h3><p>Tambahkan produk yang ingin dibeli.</p></div>}<footer>{needsHwid && <label className="cart-hwid">HWID perangkat lisensi<input value={hwid} maxLength={16} onChange={(event) => setHwid(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ''))} placeholder="16 karakter HWID" /><small>Wajib untuk produk software berlisensi.</small></label>}<span><small>Subtotal</small><b>{money(cartSubtotal(items))}</b></span><button disabled={!items.length || busy || (needsHwid && hwid.length !== 16)} onClick={() => onCheckout(needsHwid ? hwid : undefined)}>{busy ? 'Membuat invoice...' : 'Checkout Sekarang'} <ArrowRight /></button><small>Satu invoice dan satu QRIS untuk semua produk.</small></footer></>}</aside></div>;
}
